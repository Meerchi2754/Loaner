...
export default function Settings() {
  ...
  const handleExportToPDF = async () => {
    ...
    if (monthIdx === currentMonth) {
      ...
      try {
        const res = await fetch(
          `/Data/${currentYear}-${String(selectedMonth).padStart(2, "0")}.json`,
        );
    ...
    if (!filtered.length) {
      toast.error(`No transactions for ${monthName}.`);
      return;
    }
    ...
    doc.setFontSize(14);
    doc.setTextColor("#34D399"); // Green for income
    doc.text(
      `Total Income: Rs. ${totalIncome.toLocaleString("en-IN")}`,
      10,
      20,
    );
    doc.setTextColor("#F87171"); // Red for expense
    doc.text(
    ...
    <>
      <div className="min-h-screen bg-[#121212] text-white px-4 pt-8 pb-28 space-y-4">
        ...
        <div className="flex items-center gap-4 bg-[#1C1C1E] border border-white/8 rounded-2xl p-4 mb-2">
          <div
            className="w-14 h-14 rounded-full bg-linear-to-br from-emerald-400 to-blue-500
        ...
        {/* ── Section label ────────────────────────────────────────────────── */}
        <p className="text-[10px] uppercase tracking-widest text-gray-600 px-1">
          Data Management
        </p>
        ...
        >
          ...
          <p className="text-[10px] text-gray-600 mt-2 text-center">
            On Android, this opens the share sheet so you can save to Drive,
            WhatsApp, etc.
          </p>
        </SettingCard>

        {/* ── Reset ────────────────────────────────────────────────────────── */}
        <p className="text-[10px] uppercase tracking-widest text-gray-600 px-1 pt-2">
          Danger Zone
        </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
          ...
          <div className="relative bg-[#1C1C1E] border border-white/10 rounded-2xl p-6 w-full max-w-xs z-10">
            ...
            <p className="text-xs text-gray-400 text-center mb-5">
              This will permanently delete all transactions from IndexedDB. This
              cannot be undone.
            </p>
